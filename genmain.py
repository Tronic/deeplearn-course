...
def training():
    ...
    for e in epochs:
        ...
        for r in rounds:
            ...
            stats = f"lr={glr*1e6:03.0f}µ ε={noise//1000:03.0f}m {level_real:4.0%} vs{level_fake:4.0%}  {(time.perf_counter() - rtimer) / (r + .1) * len(rounds):3.0f} s/epoch"
            bar = f"E{e:1} {image_size:3}px [{'»' * (20 * r // rounds[-1]):20s}] {r+1:04d}"
            alp = 4 * " ░▒▓█"[int(alpha * 4)]
